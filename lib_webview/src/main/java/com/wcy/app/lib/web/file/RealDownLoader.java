...
import  com.wcy.app.lib.web.utils.CloseUtils;
...
public class RealDownLoader extends AsyncTask<Void, Integer, Integer> implements Observer {
    ...
    private boolean checkDownLoaderCondition() {
        ...
        if (mDownLoadTask.getLength() - mDownLoadTask.getFile().length() > SuperWebX5Utils.getAvailableStorage()) {
            LogUtils.i(TAG, " 空间不足");
            return false;
        }
        return true;
    ...
    private boolean checknet() {
        if (!mDownLoadTask.isForce()) {
            return SuperWebX5Utils.checkWifi(mDownLoadTask.getContext());
        } else {
            return SuperWebX5Utils.checkNetwork(mDownLoadTask.getContext());
        }
    ...
    protected void onPostExecute(Integer integer) {
        ...
        try {
            ...
            if (mDownLoadTask.isEnableIndicator()) {
                if (mNotify != null)
                    mNotify.cancel(mDownLoadTask.getId());
                Intent mIntent = SuperWebX5Utils.getCommonFileIntentCompat(mDownLoadTask.getContext(), mDownLoadTask.getFile());
                try {
                    if (mIntent != null) {
    ...
    private PendingIntent buildCancelContent(Context context, int id) {
        ...
        intentCancel.setAction("com.superweb.cancelled");
    ...
    public static class NotificationBroadcastReceiver extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (action.equals("com.superweb.cancelled")) {
