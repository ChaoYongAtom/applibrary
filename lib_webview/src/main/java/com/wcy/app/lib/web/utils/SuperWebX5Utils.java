...
import  com.wcy.app.lib.web.file.FileParcel;
...
import static  com.wcy.app.lib.web.SuperWebX5Config.FILE_CACHE_PATH;
import static  com.wcy.app.lib.web.SuperWebX5Config.SUPERWEB_FILE_PATH;


/**
 * utils工具类
 */
public class SuperWebX5Utils {
...
//        /*如果无法直接打开，就跳出软件列表给用户选择 */
    ...
    public static void clearWebViewAllCache(Context context, WebView webView) {
        ...
        try {
            ...
            SuperWebX5Config.removeAllCookies(null);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    public static File createFileByName(Context context, String name, boolean cover) throws IOException {
        ...
        String path = getSuperWebFilePath(context);
        if (TextUtils.isEmpty(path))
            return null;
    ...
    static String getSuperWebFilePath(Context context) {
        if (!TextUtils.isEmpty(SUPERWEB_FILE_PATH))
            return SUPERWEB_FILE_PATH;
        ...
        return SUPERWEB_FILE_PATH = mFile.getAbsolutePath();
    ...
    static Uri getUriFromFileForN(Context context, File file) {
        Uri fileUri = FileProvider.getUriForFile(context, context.getPackageName() + ".SuperWebX5FileProvider", file);
        return fileUri;
    }
    ...
    public static String getFileAbsolutePath(Activity context, Uri fileUri) {
        ...
        else if (fileUri.getAuthority().equalsIgnoreCase(context.getPackageName() + ".SuperWebX5FileProvider")) {

            String path = fileUri.getPath();
            int index = path.lastIndexOf("/");
            return getSuperWebFilePath(context) + File.separator + path.substring(index + 1, path.length());
    ...
    public static Intent getIntentCompat(Context context, File file) {
        ...
        if (context.getApplicationInfo().targetSdkVersion >= Build.VERSION_CODES.N) {
            ...
            mIntent.setDataAndType(FileProvider.getUriForFile(context, context.getPackageName() + ".SuperWebX5FileProvider", file), "application/vnd.android.package-archive");
            mIntent.addFlags(Intent.FLAG_GRANT_READ_URI_PERMISSION);
        } else {

            mIntent = SuperWebX5Utils.getFileIntent(file);
        }
